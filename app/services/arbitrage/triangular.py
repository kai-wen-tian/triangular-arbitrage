...
def calculate_profit(pair1, pair2, pair3):
    ...
    if 'ETH' in quoteCurrency or 'BTC' in quoteCurrency:
        ...
        coin1 = calculate_buy_or_sell(pair0, pair1, coin0, pair1_sell_price, pair1_buy_price,fee_percentage_pair)
        coin2 = calculate_buy_or_sell(pair1, pair2, coin1, pair2_sell_price, pair2_buy_price,fee_percentage_pair)
...
def execute_tri():
    ...
    for relationship in data['triangular_relationships']:
        try:
            ...
            if coin > 1000:
                logging.info("Trade Found")
                logging.info(f"Triangular relationship: {pair1} -> {pair2} -> {pair3}")
                logging.info(coin)
                #custom_list.append(pair1,pair2,pair3,coin)
                return coin
                break
            else:
                logging.info("Trade Not Found")
                logging.info(f"Triangular relationship: {pair1} -> {pair2}-> {pair3}")
                logging.info(coin)
                #custom_list.append(pair1,pair2,pair3,coin)
                #return (coin)
